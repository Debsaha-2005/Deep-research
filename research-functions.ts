...
): Promise<{ searchQueries: string[] } | undefined> {
  try {
    ...
    return result as { searchQueries: string[] };
  } catch (error) {
    return handleError(
      error,
      `Research planning`,
    ...
    ) as { searchQueries: string[] } | undefined;
  }
}
  ...
  try {
    ...
    activityTracker.add(
      ...
      `Analyzed collected research findings: ${
        isContentSufficient ? "Content is sufficient" : "More research is needed!"
      }`
    );
